...
object ModelProvisioning {
    ...
    const val GGUF_FILE_NAME       = "qwen3-asr-0.6b.gguf"
    ...
    fun ggufFile(context: Context): File =
        File(context.filesDir, GGUF_FILE_NAME)

    fun modelPath(context: Context, backend: QwenAsrBackend): String? = when (backend) {
        QwenAsrBackend.SAFETENSORS -> resolveSafetensorsPath(context)
        QwenAsrBackend.GGUF        -> resolveGgufPath(context)
    }

    private fun resolveSafetensorsPath(context: Context): String? {
        ModelStore.savedLocalPath(context, ModelRole.SAFETENSORS)?.let { local ->
            if (File(local, "model.safetensors").exists()) return local
        }
        val legacyDir = modelDir(context)
        return if (File(legacyDir, "model.safetensors").exists()) legacyDir.absolutePath else null
    }

    private fun resolveGgufPath(context: Context): String? {
        ModelStore.savedLocalPath(context, ModelRole.GGUF)?.let { local ->
            if (File(local).exists()) return local
        }
        val legacyFile = ggufFile(context)
        return if (legacyFile.exists()) legacyFile.absolutePath else null
    }
