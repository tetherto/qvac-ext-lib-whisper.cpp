...
def render_markdown(results: list[dict[str, Any]]) -> str:
    ...
    header = (
        "| Family | Model | Runner | OS | Wall ms (median) | Min | Max | Runs | Status | Notes |\n"
        "|---|---|---|---|---:|---:|---:|---:|---|---|\n"
    )
    rows = []
    for r in results:
        rows.append(
            "| {family} | {model} | {runner} | {os} | {median} | {min} | {max} | {runs} | {status} | {notes} |".format(
                family=r.get("family", "?"),
                model=r.get("model", "?"),
                runner=r.get("runner", "?"),
                os=r.get("os", "?"),
...
def main() -> int:
    ...
    body = title + subtitle + render_markdown(results)
