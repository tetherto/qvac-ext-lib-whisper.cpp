...
int sortformer_measure_head(const ParakeetCtcModel & model, int T_enc,
    ...
    // identical between the two weight sets. The measurement itself still uses
    // the resolved head backend's buffer type, so the force-CPU projection is
    // priced on the CPU. The normal path executes through the shared
    // scheduler, but the head is fully supported on its backend (1 split), so
    // a gallocr measurement projects the same buffer.
    ...
    int rc = 0;
    ggml_gallocr_t pricer = ggml_gallocr_new(ggml_backend_get_default_buffer_type(backend));
    if (pricer) {
        ggml_gallocr_reserve_n_size(pricer, cg, nullptr, nullptr, &out_bytes);
