...
// Pure helpers shared by the fit projector (fit.cpp) and the
// acestep-fit-params CLI (fit_cli.cpp), kept header-only and dependency-free
// so test/test_fit_cli.cpp can exercise them model-free. Adopted verbatim from
// parakeet's src/fit_util.h (the canonical fabric-speech fit helpers) so every
// engine's fit tool parses, saturates, and escapes identically; the one
// audiogen addition is parse_f32_nonnegative (guidance scales accept 0 = auto).
//
...
inline bool parse_f32_nonnegative(const char * s, float & out) {
    if (!s) return false;
    char * end = nullptr;
    const double v = std::strtod(s, &end);
    if (end == s || *end != '\0' || !std::isfinite(v) || v < 0.0) return false;
...
// Escape a string for embedding in a JSON string literal. Model names come
// from GGUF metadata and device names from the backend/driver, so neither is
