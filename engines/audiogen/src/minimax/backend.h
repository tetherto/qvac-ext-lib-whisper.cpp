...
static BackendPair backend_init(const char *) {
    if (g_backend_refs > 0) {
        ++g_backend_refs;
    ...
    BackendPair pair;
    pair.backend = backend_create_cpu();
    pair.cpu_backend = pair.backend;
    if (!pair.backend) {
...
static ggml_backend_sched_t backend_sched_new(BackendPair pair, int max_nodes) {
    ggml_backend_t backends[] = {pair.backend};
    ggml_backend_buffer_type_t buffer_types[] = {ggml_backend_get_default_buffer_type(pair.backend)};
    ggml_backend_sched_t scheduler =
        ggml_backend_sched_new(backends, buffer_types, 1, max_nodes, false, true);
    if (!scheduler) {
        throw std::runtime_error("minimax engine: CPU scheduler initialization failed");
    }
    return scheduler;
}
