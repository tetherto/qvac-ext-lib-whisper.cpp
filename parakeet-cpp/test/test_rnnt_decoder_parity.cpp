// Plain RNN-T decoder parity vs reference token IDs (NeMo dump or cross-backend).
//
// Greedy decoding is deterministic; this compares integer token IDs only.
//
// Usage:
//   test-rnnt-decoder-parity <rnnt.gguf> <wav> [<ref-dir>]
//
// Pass <ref-dir> containing token_ids.npy from
//   `scripts/dump-rnnt-reference.py --wav <wav>`
// to assert bit-identical greedy decoding against NeMo.
//
...
// Run the full pipeline (mel -> encoder -> plain RNN-T greedy) on the given
// GGUF using the requested n_gpu_layers, returning token IDs + transcript.
int transcribe_rnnt(const std::string & gguf_path,
    ...
    if (model.model_type != ParakeetModelType::RNNT) {
        std::fprintf(stderr, "  error: expected RNNT model in %s\n",
                     gguf_path.c_str());
        return 110;
    }
...
int main(int argc, char ** argv) {
    if (argc < 3) {
        std::fprintf(stderr,
            "usage: %s <parakeet-rnnt.gguf> <wav> [<ref-dir>]\n"
            "\n"
    ...
    std::fprintf(stderr, "[rnnt-decode-parity] running CPU fallback (n_gpu_layers=0)...\n");
    std::vector<int32_t> ids_cpu;
    std::string text_cpu;
    if (int rc = transcribe_rnnt(gguf_path, wav_path, 0, ids_cpu, text_cpu); rc != 0) {
        return rc;
    }
    std::fprintf(stderr, "[rnnt-decode-parity] CPU: tokens=%zu text=%.80s%s\n",
    ...
    std::fprintf(stderr, "[rnnt-decode-parity] GPU: tokens=%zu text=%.80s%s\n",
    ...
    if (!ref_dir.empty()) {
        ...
        if (load_npy_i32(p, ids_ref) != 0) {
            ...
            if (ids_ref.size() != ids_gpu.size() ||
                ...
                std::fprintf(stderr,
                    "[rnnt-decode-parity] FAIL: NeMo reference (%zu tokens) vs C++ (%zu) mismatch\n",
                    ids_ref.size(), ids_gpu.size());
                print_first_diff(ids_ref, ids_gpu);
                ok = false;
            } else {
                std::fprintf(stderr,
                    "[rnnt-decode-parity] PASS: NeMo reference matches (%zu tokens)\n",
                    ids_ref.size());
            }
        }
    }
    ...
    if (ok) {
        std::fprintf(stderr, "[rnnt-decode-parity] all checks passed\n");
        return 0;
    }
    std::fprintf(stderr, "[rnnt-decode-parity] one or more checks failed\n");
    return 1;
}
